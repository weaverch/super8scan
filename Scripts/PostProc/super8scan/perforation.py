...
class s8sPerforation:
    ...
    def perf_analyze(self, img):
        # img is the source 8 bit color tiff read from Capture directory
    ...
    def perf_cropper(self, img):
        self.myCropnwX = self.cropXOrigin
        self.myCropseX = self.cropXOrigin + self.cropXWidth
        self.myCropnwY = self.meanYImage - self.cropYBand
        self.myCropseY = self.meanYImage + self.cropYBand
        
        # Hack to deal with apparent position shift of image on film; off center from perf
        self.myCropnwY = self.myCropnwY - self.perf_shift
        self.myCropseY = self.myCropseY - self.perf_shift
        self.myCropROI = img[self.myCropnwY:self.myCropseY, self.myCropnwX:self.myCropseX]
        return self.myCropROI
